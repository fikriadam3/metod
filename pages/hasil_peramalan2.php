<?php 
	...
	$query_dpenjualan = "SELECT YEAR(penjualan.tgl_penjualan) AS tahun, MONTH(penjualan.tgl_penjualan) AS bulan, SUM(penjualandetail.jml_jual) AS jumlah_terjual FROM penjualan INNER JOIN penjualandetail ON penjualan.no_penjualan = penjualandetail.no_penjualan WHERE penjualandetail.kd_obat = '$kd_obat' AND penjualan.tgl_penjualan BETWEEN '$periode1' AND '$periode2' GROUP BY YEAR(penjualan.tgl_penjualan), MONTH(penjualan.tgl_penjualan)";
	// $query_dpenjualan = "SELECT * FROM pegawai";
